...
|CA key storage
|AES-256-GCM encrypted, stored in MongoDB. Encryption key via `LUCID_CA_ENCRYPTION_KEY` env var
...
=== Phase 1: CA Key Encryption

The CA certificate and private key are stored in MongoDB, encrypted at rest
using AES-256-GCM. The encryption key is provided via an environment variable
at API startup.
...
|`LUCID_CA_ENCRYPTION_KEY`
|Base64-encoded 32-byte AES-256 key for CA key encryption
...
IMPORTANT: This key protects the CA private key. If lost, the CA private key
cannot be decrypted and all existing agent certificates become unverifiable.
If compromised alongside the database, the CA private key is exposed. Treat it
with the same care as the CA key itself — inject it via a secrets manager,
systemd credentials, or a secrets-aware deployment platform.

==== Encryption Scheme

Each encrypted blob is stored as the concatenation of:

[source]
----
nonce (12 bytes) || ciphertext || AES-256-GCM authentication tag (16 bytes)
----

A fresh random nonce is generated for every write. The authenticated
additional data (AAD) includes the record ID to prevent ciphertext
transplantation between records.
...
pub struct DbCa {
    pub id: Uuid,
    pub cert_pem: String,              // plaintext — the CA certificate is public
    pub encrypted_private_key: Vec<u8>, // nonce || ciphertext || tag
    pub created_at: DateTime<Utc>,
}
----

Only one `DbCa` record is expected in v1. The `cert_pem` field is stored
plaintext since the CA certificate is public. Only the private key is
encrypted.
...
pub trait CaStore: Send + Sync {
    async fn get(&self) -> Result<Option<DbCa>>;
    async fn create(&self, ca: DbCa) -> Result<DbCa>;
}
----
...
==== Implementation: `EncryptedCa`

Loads the CA record from `CaStore`, decrypts the private key using the
AES-256-GCM key from `LUCID_CA_ENCRYPTION_KEY`, signs the CSR using `rcgen`,
sets CN to the agent UUID, and returns a PEM-encoded certificate valid for
24 hours.

[source,rust]
----
pub struct EncryptedCa {
    ca_store: Arc<dyn CaStore>,
    encryption_key: [u8; 32],  // loaded from LUCID_CA_ENCRYPTION_KEY at startup
}
----
...
Requires `LUCID_CA_ENCRYPTION_KEY` to be set. This command:

1. Generates a new CA certificate and Ed25519 private key using `rcgen`
2. Encrypts the private key with AES-256-GCM
3. Stores the encrypted blob + plaintext cert in MongoDB as a `DbCa` record
4. Prints the CA certificate fingerprint for verification

If a CA record already exists, the command errors unless `--force` is passed.
...
The CA private key is protected by AES-256-GCM encryption in the database.
Compromise requires both the database contents AND the `LUCID_CA_ENCRYPTION_KEY`
value. Mitigations:

- Inject `LUCID_CA_ENCRYPTION_KEY` via a secrets manager, systemd credentials,
  or a sealed secret — never commit it to version control or bake into images
- Restrict database access to the API service only
- Rotate the encryption key and re-encrypt the CA record if compromise is suspected
...
- CA key encryption: AES-256-GCM encrypt/decrypt round trip, nonce uniqueness, AAD binding
...
- [ ] Set `LUCID_CA_ENCRYPTION_KEY` env var
- [ ] Run `lucid-api generate-ca`
- [ ] Retrieve CA cert from `/.well-known/lucid/agent`
- [ ] Verify CA cert fingerprint matches DB record
...
|1 |CA key encryption + `DbCa` model + `CaStore` trait |—
|2 |CA tooling (`EncryptedCa`, `generate-ca` command) |1
...
- **External secrets manager** — Vault or GCP KMS as a CA key backend for higher-security deployments
