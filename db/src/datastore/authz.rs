...
use lucid_auth::authz::{self, AuthzStorage, permissions};
...
impl Storage for super::DataStore {
    async fn role_bind_list_for(
        ...
        opctx.authorize(&authz::Permission::new(permissions::role_bindings::LIST))?;

        let org_id = opctx.authz.organisation_id().ok_or_else(|| Error::Internal {
            internal_message: "role_bind_list_for requires an org-scoped OpContext".into(),
        })?;
        ...
        let bindings = role_bindings_dsl::role_bindings
            .filter(role_bindings_dsl::principal_type.eq(identity_type))
            .filter(role_bindings_dsl::principal_id.eq(identity_id))
            .filter(role_bindings_dsl::organisation_id.eq(org_id.into_untyped_uuid()))
            .select(RoleBinding::as_select())
            .get_results(&mut conn)
            .await
            .map_err(|e| Error::internal_anyhow(
                "failed to query role bindings".into(),
                e.into(),
            ))?;
